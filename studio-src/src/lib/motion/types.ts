...
export interface Scene {
  ...
  // Presenter-cam picture-in-picture (any template) — a small clip
  // layered in the lower third, talking over the footage.
...
export interface MotionDoc {
  ...
  fontHeading: string;
  /** CSS font-family for body copy and UI-sized text. */
  fontBody: string;
  /** CSS font-family for the "tech voice" — kickers, list markers,
      tip labels, watermark. Empty string = use fontBody. */
  fontLabel: string;
  /** Forward lean (degrees) applied to tall accent marks — tip bars and
      wipe edges. 0 = upright. A brand signature knob (e.g. -13). */
  accentSkewDeg: number;
  watermark: string;
  showGrain: boolean;
