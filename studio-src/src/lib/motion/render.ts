...
/** The "tech voice" family — kickers, markers, labels ('' = body font). */
function labelFamily(doc: MotionDoc): string {
  return doc.fontLabel || doc.fontBody;
}

/**
 * Fill a rect that leans by doc.accentSkewDeg (a parallelogram, top edge
 * shifted forward like CSS skewX). Upright when the doc doesn't set a lean.
 */
function fillAccentRect(
  ctx: CanvasRenderingContext2D,
  doc: MotionDoc,
  x: number, y: number, w: number, h: number,
) {
  const deg = doc.accentSkewDeg || 0;
  if (!deg) {
    ctx.fillRect(x, y, w, h);
    return;
  }
  const dx = (Math.tan((-deg * Math.PI) / 180) * h) / 2;
  ctx.beginPath();
  ctx.moveTo(x + dx, y);
  ctx.lineTo(x + w + dx, y);
...
/**
 * Small clip thumbnail in the lower third (e.g. the presenter talking
 * over the footage). Rounded, accent-ringed, rises in with the scene.
 */
...
function drawTextCues(
  ...
  for (const cue of cues) {
    ...
    if (cue.style === 'tip') {
      ...
      const labelFont = fontStr(700, labelPx, labelFamily(doc));
      const maxW = W * 0.52;
      ...
      // Overdraw left so the clipped edge stays covered; the right edge
      // carries the brand lean when accentSkewDeg is set.
      fillAccentRect(ctx, doc, x0 - 4 * u, y0, barW + 4 * u, plateH);
      ctx.restore();
...
function drawScene(
  ...
  if (wm) {
    ctx.save();
    ctx.globalAlpha *= 0.4;
    ctx.font = fontStr(500, 16 * u, labelFamily(doc));
    ctx.fillStyle = fg;
    ctx.textBaseline = 'alphabetic';
    const tw = ctx.measureText(wm.toUpperCase()).width + wm.length * 1.4 * u;
    drawSpacedText(ctx, wm, fontStr(500, 16 * u, labelFamily(doc)), fg, W - frame.x - tw, H - 42 * u, 1.4 * u, 'left', 1);
    ctx.restore();
  }
  ...
  if (hasKicker) {
    ...
    if (p.align === 'left') {
      ...
      if (kp > 0) {
        ...
        drawSpacedText(ctx, scene.kicker, fontStr(700, kickerPx, labelFamily(doc)), p.accent, p.anchorX + lineW + 20 * u, ky, kickerPx * 0.17, 'left', 1);
        ctx.restore();
      }
      ...
      if (kp > 0) {
        ...
        ctx.globalAlpha *= kp;
        ctx.translate((1 - kp) * 14 * u, 0);
        drawSpacedText(ctx, scene.kicker, fontStr(700, kickerPx, labelFamily(doc)), p.accent, p.anchorX - lineW - 20 * u, ky, kickerPx * 0.17, 'right', 1);
        ctx.restore();
      }
    } else if (kp > 0) {
      ctx.save();
      ...
      drawSpacedText(ctx, scene.kicker, fontStr(700, kickerPx, labelFamily(doc)), p.accent, p.anchorX, ky, kickerPx * 0.17, 'center', 1);
      ctx.restore();
    }
    y += kickerH;
  }
...
function drawListScene(
  ...
  items.forEach((item, i) => {
    ...
    const numFont = fontStr(700, 20 * u, labelFamily(doc));
    ctx.font = numFont;
    ctx.fillStyle = p.accent;
...
function drawEndcardScene(
  ...
  if (hasKicker) {
    ...
    if (kp > 0) {
      ctx.save();
      ctx.globalAlpha *= kp;
      drawSpacedText(ctx, scene.kicker, fontStr(700, kickerPx, labelFamily(doc)), p.accent, sc.W / 2, y + kickerPx * 0.8, kickerPx * 0.2, 'center', 1);
      ctx.restore();
    }
    y += parts[1];
  }
...
export function renderFrame(
  ...
  if (scene.transition === 'fade') {
    ...
    // The wipe edge leans by accentSkewDeg — the brand slash between scenes.
    // The edge sweeps from -adx to W+adx so the slanted line fully clears
    // both ends of the frame.
    const dx = (Math.tan(((-(doc.accentSkewDeg || 0)) * Math.PI) / 180) * H) / 2;
    const adx = Math.abs(dx);
    const edgeX = -adx + (W + 2 * adx) * p;
    ctx.beginPath();
    ctx.moveTo(-adx, 0);
    ctx.lineTo(edgeX + dx, 0);
    ctx.lineTo(edgeX - dx, H);
    ctx.lineTo(-adx, H);
    ctx.closePath();
    ctx.clip();
    ctx.drawImage(buf, 0, 0);
    ...
    if (p < 1) {
      ...
      fillAccentRect(ctx, doc, edgeX - 3, 0, 6, H);
    }
